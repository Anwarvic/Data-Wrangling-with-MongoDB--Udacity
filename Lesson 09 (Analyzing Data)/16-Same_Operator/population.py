...
def make_pipeline():
    ...
    match = {"$match": {"country": "India"}}  #i had to include those whose name is None, don't know why

    group = {"$group": {"_id": "$isPartOf",
                        "average": {"$avg":"$population"}}}
                        
    a_group = {"$group": {"_id" : "India Regional City Population Average",
                          "avg" : { "$avg" : "$average" }}}
